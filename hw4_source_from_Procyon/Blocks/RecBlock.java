...
public class RecBlock extends Block
{
    private int[][] matrix;
    private int row;
    private int col;
    
    public RecBlock(final int[][] matrix, final int row, final int col) {
    ...
    public void visit(final FireBall ball) {
        ...
        for (int i = 0; i < this.matrix.length; ++i) {
            for (int j = 0; j < this.matrix[i].length; ++j) {
                if (this.matrix[i][j] != 0 && (i - this.row == 1 || i - this.row == 0 || i - this.row == -1) && (j - this.col == 1 || j - this.col == 0 || j - this.col == -1)) {
                    this.matrix[i][j] = 0;
                }
            }
        }
